...
import { {{ tablePascalCase }}UpdateValidation } from "./{{ tableLowerCase }}.validation";
...
export async function handleRequest(req: Request): Promise<Response> {
    try {
        ...
        {% endfor %}
        const uuid = {{ tableCamelCase }}.uuid;

        parse({{ tablePascalCase }}UpdateValidation, { {% for column in columns %} {% if column.columnLast == false %}{{ column.columnName }}: {{ column.columnNameCamelCase }}, {% else %} {{ column.columnName }}: {{ column.columnNameCamelCase }}{% endif %}{% endfor %} });
